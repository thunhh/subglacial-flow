...
function grounding_lines_1d()
    ...
    for it in 1:nt
        ...
        # für h_neg(i = 1) bräcuhte ich i = 0 für limiter, existiert nicht, deshalb bei 2 starten und end -1 enden
        @. h_neg = h[2:end-1] + 0.5 * limiter[1:end] * (h[3:end] - h[2:end-1])
        @. h_pos = h[2:end-1] - 0.5 * limiter[1:end] * (h[3:end] - h[2:end-1])

        # 2nd order scheme for Darcy(-Weisbach) water flux
        @. q[2:end-2] = -k * 0.5 * (h_neg[1:end-1] + h_pos[2:end]) * ∇φ[2:end-1] - k * 0.5 * abs(∇φ[2:end-1]) * (h_pos[2:end] - h_neg[1:end-1])
        ...
        @. h[1:end-1] -= dt * (q[2:end] - q[1:end-1]) / dx
        h[end] = 4.2e3
        # update plot
        if it % nvis == 0
