...
function grounding_lines_1d()
    ...
    tcur = 0.0
    for it in 1:nt
        @show length(q[2:end-2])
        @show length(h[1:end-1])
        @show length(h[2:end])
        @show length(∇φ)
        @show size(abs.(∇φ).^betha)
        # (overburden) hydraulic potential = overburden pressure + elevation potential + water pressure
        @. φ  = σnn + ρʷg * (B + h)
        @. ∇φ = (φ[2:end] - φ[1:end-1]) / dx
        # Darcy(-Weisbach) water flux -> nonlinear
        # @. q[2:end-2] = -k * 0.5 * (h[1:end-2]^alpha * abs(∇φ[1:end-1])^betha  + h[2:end-1]^alpha * abs(∇φ[2:end])^betha) * ∇φ[1:end-1] - k * 0.5 * abs(∇φ[1:end-1]) * (h[2:end-1] - h[1:end-2])
        println(h[end-4: end])
        println(∇φ[end-4: end])
        println(q[end-4: end])

        @assert all(h .>= 0.0)
        @assert all(abs.(∇φ) .>= 0.0)
        @. q[2:end-2] = -k * 0.5 * (h[1:end-1].^alpha * abs(∇φ).^betha  + h[2:end].^alpha * abs(∇φ).^betha) * ∇φ - k * 0.5 * abs(∇φ) * (h[2:end] - h[1:end-1])
